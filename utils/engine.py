    ...
    def on_end_epoch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        if display:
            if training:
                ...
                print('per-class AP:', self.state['ap_meter'].value().detach().numpy()*100)
                print('Test: \t Loss {loss:.4f}\t mAP {map:.3f}'.format(loss=loss, map=map))
                OP, OR, OF1, CP, CR, CF1, EP, ER, EF1 = self.state['ap_meter'].overall()
                OP_k, OR_k, OF1_k, CP_k, CR_k, CF1_k, EP_k, ER_k, EF1_k = self.state['ap_meter'].overall_topk(3)
                ...
                with open('results.txt', 'a+') as f:
                    result = 'weights: {weights:s} \ntime: {time:s} \nresults: map | cp | cr | cf1 | ep | er | ef1 | op | or | of1 \n{map:.1f} & {cp:.1f} & {cr:.1f} & {cf1:.1f} & {ep:.1f} & {er:.1f} & {ef1:.1f} & {op:.1f} & {o_r:.1f} & {of1:.1f} \n'.format(weights=self.state['resume'], time=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), map=map, cp=CP*100, cr=CR*100, cf1=CF1*100, ep=EP*100, er=ER*100, ef1=EF1*100, op=OP*100, o_r=OR*100, of1=OF1*100)
                    f.write(result)
        return map
    def on_start_batch(self, training, model, criterion, data_loader, optimizer=None, display=True):
...
class MultiLabelMAPEngine(MultiLabelMAPEngine):
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
